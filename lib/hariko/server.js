...
HarikoServer.prototype = {
  setup: function (entries) {
    ...
    if (this.options.proxy) {
      this.app.all('*', proxy(this.options.proxy, entries));
    }
    this.app.use(bodyParser.urlencoded({ extended: true }));
    this.app.use(bodyParser.json());
    this.app.use(selfAssertive());
    this.app.use(routing(entries));
    if (this.options.proxy) {
      this.app.all('*', proxy(this.options.proxy));
    }
  },
  start: function (entries, cb) {
    ...
    ;
    this._server.listen(this.options.port, this.options.host || 'localhost');
  },
  stop: function () {
    this._server.close();
    this._server = null;
