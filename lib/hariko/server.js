...
const ASSERTIVE_NAME = 'Hariko API Server'
...
export class BakerServer {
  ...
  start () {
    this.app.listen(this.options.port, this.options.host || 'localhost', () => {
      console.log('runnning ... http://%s:%s', this.options.host, this.options.port);
    });
  }
}

export function create (entries, options) {
  return new BakerServer(entries, options);
}
